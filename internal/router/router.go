...
func Setup(r *gin.Engine, deps Deps) {
	...
	v1.POST("/messages", deps.Bridge.Route(utils.APIMessages))
	...
	admin := r.Group("/admin")
	{
		...
		apiGroup := admin.Group("/api", handler.AdminAuthMiddleware(deps.AuthCache))
		{
			...
			apiGroup.GET("/antigravity", deps.Admin.ListAntigravity)
			apiGroup.POST("/antigravity", deps.Admin.BatchCreateAntigravity)
			...
			apiGroup.DELETE("/logs/:provider", deps.Admin.ClearLogs)
			apiGroup.POST("/providers/:provider/quota", deps.Admin.RefreshQuota)
