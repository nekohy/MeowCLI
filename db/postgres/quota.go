...
func (s *Store) ListAvailableCodex(ctx context.Context) ([]db.ListAvailableCodexRow, error) {
	...
	for i, row := range rows {
		resolved[i] = listAvailableCodexRowTo(row.ID, row.PlanType, row.Quota5h, row.Quota7d, row.Quota1mo, row.QuotaSpark5h, row.QuotaSpark7d, row.QuotaSpark1mo, tsTo(row.Reset5h), tsTo(row.Reset7d), tsTo(row.Reset1mo), tsTo(row.ResetSpark5h), tsTo(row.ResetSpark7d), tsTo(row.ResetSpark1mo), tsTo(row.ThrottledUntil), tsTo(row.ThrottledUntilSpark), tsTo(row.SyncedAt))
	}
