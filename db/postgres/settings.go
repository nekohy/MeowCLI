...
func (s *Store) ListSettings(ctx context.Context) ([]db.Setting, error) {
	...
	for i, row := range rows {
		resolved[i] = settingTo(row.Key, row.Value, tsTo(row.UpdatedAt))
	}
	return resolved, nil
}

func (s *Store) UpsertSetting(ctx context.Context, arg db.UpsertSettingParams) (db.Setting, error) {
	row, err := s.queries.UpsertSetting(ctx, sqlcpostgres.UpsertSettingParams{
		Key:   arg.Key,
		Value: arg.Value,
	})
	if err != nil {
	...
	return settingTo(row.Key, row.Value, tsTo(row.UpdatedAt)), nil
}
