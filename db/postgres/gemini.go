...
func (s *Store) SetGeminiCLIThrottled(ctx context.Context, credentialID string, throttledUntil time.Time) error {
	return wrapError(s.queries.SetGeminiCLIThrottled(ctx, sqlcpostgres.SetGeminiCLIThrottledParams{
...
func (s *Store) ListAvailableGeminiCLI(ctx context.Context) ([]db.ListAvailableGeminiCLIRow, error) {
	...
	for i, row := range rows {
		resolved[i] = listAvailableGeminiCLIRowTo(row.ID, row.Email, row.ProjectID, row.PlanType, tsTo(row.ThrottledUntil), tsTo(row.SyncedAt))
	}
	return resolved, nil
}
